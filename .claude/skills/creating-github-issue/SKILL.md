...
> Issue を切り出す前に（`spec.md` が存在する場合）**必ず `spec.md` の `[NEEDS CLARIFICATION]` を解消する**（Constitution 原則 8）。
...
- **完了条件 (`acceptance_criteria`)**: 客観的に判定可能。SDD タスクは spec.md の受入基準と矛盾しないこと（新規 feature 立ち上げ等で `spec.md` が未作成の場合は、ヒアリング内容や構想資料に基づき定義すること）。一般タスクは成果物の状態や満たすべき要件を客観的に記述すること（具体的な書き方は本文テンプレートの「完了条件」ガイドライン参照）
...
- **完了条件**: 「完了」とみなされる客観的条件。SDD タスクは **spec.md の受入基準（acceptance criteria）と矛盾しないこと**（新規 feature 立ち上げ等で `spec.md` が未作成の場合は、ヒアリング内容や構想資料に基づき定義すること）。一般タスクは、達成すべき具体的な成果物の状態や満たすべき要件（例: ドキュメントの特定セクションの追加、ツールの特定コマンドの正常動作など）を客観的に判定可能な形で記述する。
