...
`ls docs/specs/` で既存 feature の一覧を取得し、Step 1 の概要と最も整合する feature を選定する。既存に該当がなく新規 feature が必要な場合、または曖昧な場合は `AskUserQuestion` で確認する。なお、新規 feature の立ち上げ等で `spec.md` / `plan.md` がまだ存在しない場合は、以降のファイル読み込み（2A.2）をスキップし、Step 1 の概要およびユーザーへのヒアリング内容（または `docs/research/` などの構想資料）のみをソースとして Issue を作成する。この場合、2A.2 全体（`[NEEDS CLARIFICATION]` 解消の前提を含む）は対象 `spec.md` が未作成のため適用しない。
...
| タスク種別                                  | 主に参照する資料                                              |
| ------------------------------------------- | ------------------------------------------------------------- |
| Claude Code 関連（skill / subagent など）   | `.claude/skills/`、`.claude/agents/`（存在する場合）、既存の類似定義 |
...
- **完了条件 (`acceptance_criteria`)**: 客観的に判定可能。SDD タスクは spec.md の受入基準と矛盾しないこと。一般タスクは成果物の状態や満たすべき要件を客観的に記述すること（具体的な書き方は本文テンプレートの「完了条件」ガイドライン参照）
...
- **完了条件**: 「完了」とみなされる客観的条件。SDD タスクは **spec.md の受入基準（acceptance criteria）と矛盾しないこと**。一般タスクは、達成すべき具体的な成果物の状態や満たすべき要件（例: ドキュメントの特定セクションの追加、ツールの特定コマンドの正常動作など）を客観的に判定可能な形で記述する。
...
- **参考パターン**: 既存の実装パターンや SDD アーティファクトへの参照（例: `docs/guides/spec-driven-development.md`、`docs/research/` の該当章）。一般タスクでは類似の既存定義（例: `.claude/skills/` の他 skill、`.claude/agents/`（存在する場合）の既存 subagent）を参照する。
